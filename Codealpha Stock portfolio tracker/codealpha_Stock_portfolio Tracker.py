...
def remove_stock():
    ...
    if not Portfolio:
        ...
        return
    symbol = input("Enter the Stock Name To Remove = ").strip().upper()
    if symbol not in Portfolio:
        print(f"You Don't Own any share of {symbol}.")
        return

    qty_input = input(f"Enter Quantity of {symbol} shares to remove= ")
    try:
...
def add_stock():
    ...
    symbol = input("Enter the Stock Name = ").strip().upper()
    print(symbol)

    if symbol not in stock:
        print(f"Error {symbol} is not in list ")
        return
    qty_input = input(f"Enter Quantity of {symbol} shares to add = ")
    try:
        quantity = int(qty_input)
        if quantity <= 0:
            print("Quantity should me in positive")
